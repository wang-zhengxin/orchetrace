...
node scripts/verify-desktop-artifact.mjs \
  --target aarch64-apple-darwin \
  --bundle-root target/aarch64-apple-darwin/release/bundle
...
Tauri 的 Runtime Diagnostics 提供 `Storage Doctor`：按需检查 SQLite integrity、外键、Canonical Event payload、索引一致性和 checkpoint 状态。`ARM REPAIR` 只在检查结果可修复且 Ingest 已停止时启用，第二次确认后仅重建可丢弃的 checkpoint 与 Web/TUI 投影；`EXPORT RUN` 将当前 Run 的 Canonical Event 写入应用数据目录下的 `exports/`，文件名自动生成且不会覆盖已有文件。命令行仍可使用下方的 `otrace repair` / `otrace export`。
...
- npm CLI 已进入四平台安装生命周期门禁，DMG/DEB/MSI 已有最终内容检查，macOS DMG 已有隔离安装与启动烟测；Developer ID/Windows 正式证书、macOS notarization、自动更新以及桌面跨版本/Homebrew 实体升级回滚验证尚未完成；
