...
标签发布还会在全新 macOS runner 上对生成的 Formula 和 Cask 执行 `brew style` 与实体生命周期：Formula 安装后验证 `orche`、`otrace`，Cask 安装到临时 Applications 后验证隔离桌面启动，两者最后都必须成功卸载。通过该门禁后才允许更新项目 Tap。Homebrew 的跨版本升级/回滚仍属于后续发布验收。
