...
const usage = `Usage: orchetrace-antigravity-auto [options]

Passively discover Antigravity CLI transcript JSONL files and stream canonical
events to Orchetrace. This command never launches or controls agy.

Options:
  --sessions-dir <path>  Antigravity brain directory
                         (default: ~/.gemini/antigravity-cli/brain)
  --state-dir <path>     ACK cursor directory
                         (default: .orchetrace/antigravity-auto)
  --hook-events <path>   lifecycle hook mailbox
                         (default: ~/.orchetrace/antigravity-hooks.jsonl)
  ...
  --include-existing     observe every existing Antigravity session
...
async function main(): Promise<void> {
  ...
  let lastStatus = "";
  const discovery = new AntigravityAutoDiscovery(sink, {
    ...
    onDiagnostic: report,
    onStatus: (status) => {
      const line = `antigravity auto-discovery: ${status.observedSessions} observed / ${status.discoveredSessions} discovered / ${status.hookEvents} hook event(s)`;
      if (line !== lastStatus) process.stderr.write(`${line}\n`);
      lastStatus = line;
    },
  });
  const initial = await discovery.start();
  process.stderr.write(`watching ${initial.sessionsDir}; ${initial.observedSessions} active agy session(s) attached\n`);
  await waitForSignal();
...
function parseArguments(args: string[]): Options {
  const options: Options = {
    sessionsDir: resolve(homedir(), ".gemini/antigravity-cli/brain"),
    stateDir: resolve(".orchetrace/antigravity-auto"),
