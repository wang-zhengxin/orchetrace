...
const usage = `Usage: orchetrace-dsh-auto [options]

Passively discover DeepSeek Harness Zstandard session persistence and stream it
to Orchetrace. The in-process Cordis observer can run alongside this fallback.

Options:
  --sessions-dir <path>  Harness sessions directory (default: ~/.dsh/sessions)
  --state-dir <path>     ACK cursor directory (default: .orchetrace/dsh-auto)
  ...
  --include-existing     observe every existing Harness session
...
async function main(): Promise<void> {
  ...
  const discovery = new DshAutoDiscovery(sink, {
    ...
    onDiagnostic: report,
    onStatus: (status) => {
      const line = `harness auto-discovery: ${status.observedSessions} observed / ${status.discoveredSessions} discovered`;
      if (line !== lastStatus) process.stderr.write(`${line}\n`);
      lastStatus = line;
    },
  });
  const initial = await discovery.start();
  process.stderr.write(`watching ${initial.sessionsDir}; ${initial.observedSessions} active Harness session(s) attached\n`);
  await waitForSignal();
...
function parseArguments(args: string[]): Options {
  const options: Options = {
    sessionsDir: resolve(homedir(), ".dsh/sessions"),
    stateDir: resolve(".orchetrace/dsh-auto"),
    host: "127.0.0.1",
    port: 43117,
    scanMs: 1_000,
